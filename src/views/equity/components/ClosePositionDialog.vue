...
<template>
  <el-dialog :model-value="modelValue" top="var(--ft-dialog-top)" width="960px" :teleported="false" :show-close="false" class="close-position-dialog" @update:model-value="emit('update:modelValue', $event)">
    ...
    <div v-if="position" class="close-position-layout">
      ...
      <section class="close-order-side">
        <div class="close-position-metrics"><header><span>持仓概览</span><small>可平数量以当前可用数量为准</small></header><div><span>持仓股数</span><b>{{ number(position.qty) }} 股</b></div><div><span>持仓市值(万)</span><b>{{ marketValueWan }}</b></div><div><span>可平股数</span><b>{{ number(maxSell) }} 股</b></div><div><span>成本价</span><b>{{ money(position.cost) }}</b></div></div>
        <div class="close-order-form order-form">
          ...
          <section class="price-block"><template v-if="orderType === 'limit'"><div class="field-caption"><label>委托价格</label></div><div class="price-input-row"><el-input-number v-model="price" placeholder="请输入" :step=".01" :precision="2" :controls="false" /><span class="price-stepper"><button type="button" aria-label="增加委托价格" @click="stepPrice(1)"><el-icon><ArrowUp /></el-icon></button><button type="button" aria-label="减少委托价格" @click="stepPrice(-1)"><el-icon><ArrowDown /></el-icon></button></span></div></template><template v-else><div class="field-caption"><span>委托价格</span></div><div class="market-price-note"><b>以市场价格成交</b></div></template></section>
          <section class="quantity-block"><div class="field-caption"><label>{{ quantityMode === 'quantity' ? '委托数量' : '委托金额' }}</label><el-radio-group v-model="quantityMode" class="quantity-mode" size="small"><el-radio-button label="quantity">数量</el-radio-button><el-radio-button label="amount">金额</el-radio-button></el-radio-group></div><div v-if="quantityMode === 'quantity'" class="quantity-input-row"><el-input-number v-model="quantity" placeholder="请输入" :step="unit === 'wan' ? 1 : 100" :precision="0" :controls="false" /><el-select v-model="unit" aria-label="平仓数量单位" popper-class="variant-popper quantity-unit-popper"><el-option label="股" value="shares" /><el-option label="万股" value="wan" /></el-select></div><div v-else class="quantity-input-row amount-input-row"><el-input-number v-model="displayedAmount" placeholder="请输入" :step="amountUnit === 'wan' ? 1 : 1000" :precision="2" :controls="false" /><el-select v-model="amountUnit" aria-label="平仓金额单位" popper-class="variant-popper quantity-unit-popper"><el-option label="元" value="yuan" /><el-option label="万元" value="wan" /></el-select></div></section>
          <el-slider v-model="fraction" :step="1" :marks="{0:'0%',25:'25%',50:'50%',75:'75%',100:'100%'}" @input="size" />
          <div class="close-capacity capacity-pair"><div></div><div>最大可卖<b>{{ number(maxSell) }} 股</b></div></div>
          <div class="trade-buttons"><el-button class="sell-action" :disabled="!canSubmit" @click="submit">卖出</el-button></div>
          <div class="close-estimates order-totals"><div><span>预估卖出金额</span><b>{{ shares ? `${money(shares * estimatedPrice)} CNY` : '--' }}</b></div><div><span>预估卖出股数</span><b>{{ shares ? `${number(shares)} 股` : '--' }}</b></div></div>
        </div>
      </section>
    </div>
  </el-dialog>
