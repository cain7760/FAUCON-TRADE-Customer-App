...
<template>
  <el-dialog :model-value="modelValue" top="var(--ft-dialog-top)" width="620px" :teleported="false" :show-close="false" class="close-position-dialog" @update:model-value="emit('update:modelValue', $event)">
    ...
    <div v-if="position && account" class="close-position-layout">
      <section class="close-position-metrics"><header>持仓概览</header><div><span>持仓股数</span><b>{{ number(position.qty) }} 股</b></div><div><span>持仓市值(万)</span><b>{{ marketValueWan }}</b></div><div><span>可平股数</span><b>{{ number(position.available) }} 股</b></div><div><span>成本价</span><b>{{ money(position.cost) }}</b></div></section>
