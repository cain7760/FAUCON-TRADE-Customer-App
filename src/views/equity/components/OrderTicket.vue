...
const props = defineProps({ instruments: Array, accounts: Array, symbol: Object, account: Object, quote: Object, paused: Boolean, closeMode: Boolean })
const emit = defineEmits(['select', 'account-select', 'order'])
const executionType = ref('lowTouch'), orderType = ref('limit'), price = ref(), quantity = ref(), amount = ref(), quantityMode = ref('quantity'), fraction = ref(0), note = ref('')
...
const commonValid = computed(() => !!displaySymbolCode.value && shares.value > 0 && shares.value % 100 === 0 && (!props.closeMode || shares.value <= maxSell.value) && (orderType.value === 'market' || price.value > 0))
const canBuy = computed(() => !props.paused && commonValid.value)
const canSell = computed(() => !props.paused && commonValid.value)
const confirmationTitle = computed(() => {
  if (!snapshot.value) return '确认委托'
  const action = snapshot.value.side === 'buy' ? '买入' : '卖出'
  const value = quantityMode.value === 'amount' ? `${money(amount.value || 0)} 元` : `${number(snapshot.value.quantity)} 股`
...
function clearInstrument() { displaySymbolCode.value = null; emit('select', null); search.value = ''; price.value = null; note.value = ''; fraction.value = 0; confirming.value = false; nextTick(() => { quantity.value = undefined; amount.value = undefined; quantityInputKey.value += 1 }) }
...
watch(() => props.symbol.code, code => { displaySymbolCode.value = code; recentCodes.value = [code, ...recentCodes.value.filter(item => item !== code)].slice(0, 4); price.value = props.symbol.price; resetOrder() })
watch(() => props.account.id, resetOrder)
watch([orderType, unit, quantityMode], reset)
watch(executionType, resetOrder)
...
function size(value) { const q = Math.floor((props.closeMode ? maxSell.value : maxBuy.value) * value / 100 / 100) * 100; if (quantityMode.value === 'amount') amount.value = Number((q * estimatedPrice.value).toFixed(2)); else quantity.value = unit.value === 'wan' ? Math.floor(q / 10000) : q }
...
function preview(side) {
  ...
  const id = Date.now()
  snapshot.value = { id, orderNo: `WT${id}`, account: props.account.id, code: props.symbol.code, name: props.symbol.name, executionType: executionType.value, note: note.value.trim(), type: orderType.value, side, quantity: shares.value, price: orderType.value === 'limit' ? price.value : null, estimate: estimatedPrice.value * shares.value, status: '待报', runStatus: '运行中', openClose: props.closeMode ? '平' : '开', attribute: `${orderType.value === 'limit' ? '限价' : '市价'}·${quantityMode.value === 'quantity' ? '数量' : '金额'}`, orderValueNumber: quantityMode.value === 'quantity' ? shares.value : amount.value, orderValue: quantityMode.value === 'quantity' ? `${number(shares.value)} 股` : `${money(amount.value || 0)} CNY`, filledQuantity: 0, filledPrice: null }
...
<template>
  <div class="order-form" :class="{ 'is-paused': paused, 'is-manual': !closeMode && executionType === 'highTouch' }">
    ...
    <section v-if="!closeMode" class="execution-type-block">
      <div class="field-caption"><span>订单类型</span></div>
