...
function preview(side) {
  ...
  const id = Date.now()
  snapshot.value = { id, orderNo: `WT${id}`, account: props.account.id, code: props.symbol.code, name: props.symbol.name, executionType: executionType.value, note: note.value.trim(), type: orderType.value, side, quantity: shares.value, price: orderType.value === 'limit' ? price.value : null, estimate: estimatedPrice.value * shares.value, status: '待报', runStatus: '正常', openClose: props.closeMode ? '平' : '开', attribute: `${orderType.value === 'limit' ? '限价' : '市价'}·${quantityMode.value === 'quantity' ? '数量' : '金额'}`, orderValueNumber: quantityMode.value === 'quantity' ? shares.value : amount.value, orderValue: quantityMode.value === 'quantity' ? `${number(shares.value)} 股` : `${money(amount.value || 0)} CNY`, filledQuantity: 0, filledPrice: null }
