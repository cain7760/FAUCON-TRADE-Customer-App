...
<template>
  <div class="order-form" :class="{ 'is-paused': paused }">
    ...
    <section class="quantity-block">
      ...
      <div class="capacity-pair"><div>最大可买<b>{{ number(maxBuy) }} 股</b></div><div>最大可卖<el-tooltip content="最大可卖(名本) = 持仓均价*卖出股数（卖出股数暂不支持碎股，不满足1手按照1手处理，向下取整）" placement="top" popper-class="order-help-popper"><button type="button" class="total-help capacity-help" aria-label="最大可卖说明"><el-icon><InfoFilled /></el-icon></button></el-tooltip><b>{{ number(maxSell) }} 股</b></div></div>
    </section>
