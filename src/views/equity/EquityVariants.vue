      ...
      <section class="positions-pane workspace-panel">
        ...
        <template v-if="tab==='positions'">
          ...
          <TradingTable ref="table" class="original-fields" :data="sortedRows" height="100%" :fit="false" row-key="id" empty-text="无匹配持仓，请调整或重置筛选">
            ...
            <template v-for="columnKey in visibleColumnKeys" :key="columnKey">
              ...
              <el-table-column v-else-if="columnKey === 'executionType'" prop="executionType" label="订单类型" width="64"><template #default="{row}"><span class="execution-type-tag" :class="row.executionType === 'highTouch' ? 'is-high-touch' : 'is-low-touch'">{{ row.executionType === 'highTouch' ? '手工单' : '系统单' }}</span></template></el-table-column>
              <el-table-column v-else-if="columnKey === 'code'" prop="code" width="80"><template #header><SortHeader label="标的代码" :direction="sortState.prop === 'code' ? sortState.direction : null" @sort="toggleSort('code')" /></template><template #default="{row}">{{ row.code }}.{{ row.market }}</template></el-table-column>
              <el-table-column v-else-if="columnKey === 'name'" prop="name" width="76"><template #header><SortHeader label="标的名称" :direction="sortState.prop === 'name' ? sortState.direction : null" @sort="toggleSort('name')" /></template></el-table-column>
              ...
              <el-table-column v-else-if="columnKey === 'valueWan'" prop="valueWan" width="72" align="right" header-align="right"><template #header><SortHeader label="市值(万)" numeric :direction="sortState.prop === 'valueWan' ? sortState.direction : null" @sort="toggleSort('valueWan')" /></template><template #default="{row}">{{ money(row.valueWan) }}</template></el-table-column>
              <el-table-column v-else-if="columnKey === 'marginOccupied'" prop="marginOccupied" width="88" align="right" header-align="right"><template #header><SortHeader label="保证金占用" numeric :direction="sortState.prop === 'marginOccupied' ? sortState.direction : null" @sort="toggleSort('marginOccupied')" /></template><template #default="{row}">{{ money(row.marginOccupied) }}</template></el-table-column>
        ...
        <template v-else-if="tab==='orders'">
          <div class="order-filters">
            ...
            <el-tooltip content="导出当前筛选结果" placement="top"><button class="export-orders" aria-label="导出委托记录" @click="exportOrders"><el-icon><svg class="export-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 15V3m0 0L7.5 7.5M12 3l4.5 4.5M5 11v8a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-8"/></svg></el-icon></button></el-tooltip>
          </div>
          <TradingTable class="original-fields orders-table" :data="sortedOrders" height="100%" :fit="false" empty-text="暂无委托记录">
            <el-table-column type="index" label="序号" width="44" fixed="left" align="center" header-align="center" class-name="order-index-column" label-class-name="order-index-column" />
            <template v-for="columnKey in orderVisibleColumnKeys" :key="columnKey">
              <el-table-column v-if="columnKey === 'code'" prop="code" label="标的代码" width="68"/>
              <el-table-column v-else-if="columnKey === 'name'" prop="name" label="标的名称" width="68"/>
              <el-table-column v-else-if="columnKey === 'status'" prop="status" label="状态" width="68"><template #default="{row}"><span class="order-status" :class="`is-${orderStatusVisual(row.status).tone}`"><el-icon><component :is="orderStatusVisual(row.status).icon" /></el-icon>{{ row.status }}</span></template></el-table-column>
              <el-table-column v-else-if="columnKey === 'openClose'" prop="openClose" label="开平" width="48"><template #default="{row}">{{ row.openClose === '开' ? '开仓' : '平仓' }}</template></el-table-column>
              <el-table-column v-else-if="columnKey === 'side'" label="买卖" width="48"><template #default="{row}"><span :class="row.side==='buy'?'up':'down'">{{ row.side==='buy'?'买入':'卖出' }}</span></template></el-table-column>
              ...
              <el-table-column v-else-if="columnKey === 'price'" prop="price" width="76" align="right" header-align="right"><template #header><SortHeader label="委托价格" numeric :direction="orderSortState.prop === 'price' ? orderSortState.direction : null" @sort="toggleOrderSort('price')" /></template><template #default="{row}">{{ row.price === null ? '--' : money(row.price) }}</template></el-table-column>
              <el-table-column v-else-if="columnKey === 'orderValueNumber'" prop="quantity" width="76" align="right" header-align="right"><template #header><SortHeader label="委托数量" numeric :direction="orderSortState.prop === 'quantity' ? orderSortState.direction : null" @sort="toggleOrderSort('quantity')" /></template><template #default="{row}">{{ number(row.quantity) }}</template></el-table-column>
              <el-table-column v-else-if="columnKey === 'filledQuantity'" prop="filledQuantity" width="76" align="right" header-align="right"><template #header><SortHeader label="成交数量" numeric :direction="orderSortState.prop === 'filledQuantity' ? orderSortState.direction : null" @sort="toggleOrderSort('filledQuantity')" /></template><template #default="{row}">{{ number(row.filledQuantity || 0) }}</template></el-table-column>
              ...
              <el-table-column v-else-if="columnKey === 'account'" prop="account" label="下单账户" width="116"><template #default="{row}">{{ accountLabel(row.account) }}</template></el-table-column>
              ...
              <el-table-column v-else-if="columnKey === 'executionType'" prop="executionType" label="订单类型" width="68"><template #default="{row}"><span class="execution-type-tag" :class="row.executionType === 'highTouch' ? 'is-high-touch' : 'is-low-touch'">{{ row.executionType === 'highTouch' ? '手工单' : '系统单' }}</span></template></el-table-column>
        ...
        <template v-else>
          <div class="order-filters">
            ...
            <el-select v-model="tradeFilters.attribute" aria-label="委托方式筛选" popper-class="variant-popper" :offset="2"><el-option label="委托方式" value="ALL"/><el-option label="限价单" value="limit"/><el-option label="市价单" value="market"/></el-select>
