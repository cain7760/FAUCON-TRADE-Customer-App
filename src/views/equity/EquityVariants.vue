        ...
        <template v-else-if="tab==='orders'">
          ...
          <TradingTable class="original-fields orders-table" :data="sortedOrders" height="100%" empty-text="暂无委托记录">
            ...
            <template v-for="columnKey in orderVisibleColumnKeys" :key="columnKey">
              ...
              <el-table-column v-else-if="columnKey === 'openClose'" prop="openClose" label="开平" width="56"><template #default="{row}">{{ row.openClose === '开' ? '开仓' : '平仓' }}</template></el-table-column>
              <el-table-column v-else-if="columnKey === 'side'" label="买卖" width="56"><template #default="{row}"><span :class="row.side==='buy'?'up':'down'">{{ row.side==='buy'?'买入':'卖出' }}</span></template></el-table-column>
              <el-table-column v-else-if="columnKey === 'attribute'" prop="attribute" label="委托方式" width="72"><template #default="{row}">{{ row.type === 'market' ? '市价单' : '限价单' }}</template></el-table-column>
              ...
              <el-table-column v-else-if="columnKey === 'canceledValue'" label="撤单数量/金额" width="112" align="right" header-align="right"><template #default="{row}">{{ canceledOrderValue(row) }}</template></el-table-column>
              ...
              <el-table-column v-else-if="columnKey === 'marginRate'" prop="marginRate" label="保证金率" width="72" align="right" header-align="right"><template #default="{row}">{{ row.marginRate || 0 }}%</template></el-table-column>
              ...
              <el-table-column v-else-if="columnKey === 'orderNo'" prop="orderNo" label="订单编号" width="126"/>
