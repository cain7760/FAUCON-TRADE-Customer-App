...
import { ArrowDownBold, Bell, CaretBottom, ChatDotRound, Search, Document, Setting, InfoFilled, Upload, View, Hide, Clock, CircleCheck, CircleClose, Close, EditPen, List, More } from '@element-plus/icons-vue'
        ...
        <template v-if="tab==='positions'">
          ...
          <TradingTable ref="table" class="original-fields" :data="sortedRows" height="100%" row-key="id" empty-text="无匹配持仓，请调整或重置筛选">
            ...
            <template v-for="columnKey in visibleColumnKeys" :key="columnKey">
              <el-table-column v-if="columnKey === 'direction'" prop="direction" label="多空" width="48" align="center" header-align="center" class-name="direction-column" label-class-name="direction-column"><template #default="{row}"><span class="direction-chip" :class="row.direction === '多' ? 'long' : 'short'"><img :src="assetUrl(row.direction === '多' ? 'direction-long.svg' : 'direction-short.svg')" alt="">{{ row.direction === '多' ? '做多' : '做空' }}</span></template></el-table-column>
