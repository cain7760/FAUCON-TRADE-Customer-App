...
import { ArrowDownBold, Bell, CaretBottom, ChatDotRound, Search, Fold, Expand, Document, Setting, InfoFilled, Download, View, Hide, Clock, CircleCheck, CircleClose, Close, EditPen, List, More } from '@element-plus/icons-vue'
...
<template>
  <main class="variants-app" :class="`version-${variant}`">
    ...
    <el-dialog v-model="messageCenterVisible" width="760px" align-center class="message-center-dialog" :show-close="false">
      ...
      <div class="message-center-layout"><nav class="message-category-tabs" aria-label="消息分类"><button v-for="category in messageCategories" :key="category" :class="{ active: messageCategory === category }" @click="messageCategory=category"><el-icon class="message-category-icon"><More v-if="category === '全部'" /><Bell v-else-if="category === '通知'" /><ChatDotRound v-else-if="category === '消息'" /><List v-else /></el-icon><span>{{ category }}</span><i v-if="categoryUnreadCount(category)">{{ categoryUnreadCount(category) }}</i></button></nav>
        <section class="message-list" aria-label="消息列表"><header><b>{{ messageCategory }}</b><span v-if="categoryUnreadCount(messageCategory)" class="message-list-actions"><strong>{{ categoryUnreadCount(messageCategory) }}</strong> 条未读<button type="button" @click="markAllMessagesRead">全部标为已读</button></span></header><article v-for="message in filteredMessages" :key="message.id" class="message-item" :class="[`is-${message.category}`, { unread: message.unread, expanded: messageIsExpanded(message) }]" @click="markMessageRead(message)"><span class="message-type-icon"><el-icon><Bell v-if="message.category === '通知'" /><ChatDotRound v-else-if="message.category === '消息'" /><List v-else /></el-icon></span><div class="message-copy"><span class="message-title-row"><b>{{ message.title }}</b><em>{{ message.category }}</em></span><p>{{ message.content }}</p><button v-if="needsExpansion(message)" type="button" @click.stop="toggleMessageExpansion(message)">{{ messageIsExpanded(message) ? '收起' : '展开全部' }}</button></div><time>{{ message.time }}</time><i v-if="message.unread" aria-label="未读"></i></article><p v-if="!filteredMessages.length" class="message-empty">当前分类暂无消息</p></section>
      </div>
    </el-dialog>
